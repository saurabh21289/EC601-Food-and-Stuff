...
if __name__ == "__main__":
	...
	#As the data stored in the file is bunchs of lists, so we can add the data
	#like this to the dictionary. As the dictionary uses the hash table, it's
	#much more faster to search thourgh the data to find the wanted category
	# guess_it = ReviewCategoryClassifier(input_file)
	# for text in sys.stdin:
		# guesses = guess_it.classify(text)
	...
	# outfile=open('result_new.json','w')
	for guess, prob in best_guesses:
		# print 'Category: "%s" - %.2f%% chance' % (guess, prob * 100)
		result[guess][0][2]=(result[guess][0][2]).encode('utf-8')
		# use utf-8 to encode the string, or python2.7 uses defaulted ascii
		# and cannot print characters besides english
		temp = ', '.join(str(item) for item in result[guess][0])
		temp = re.sub(r'\n','',temp)
		print temp, round(prob*100,2),'%'
		# for item in result[guess][0]:
		#Now we print first the name and stars of the corresponding list
		#if want to output to a file:
	# 	for item in result[guess][0]:
	# 		outfile.write('%s'%(item))
	# 		outfile.write(' ')
	# 	outfile.write('%s%s'%(round(prob*100,2),'%'))
	# 	outfile.write('\n')
	# outfile.close()
