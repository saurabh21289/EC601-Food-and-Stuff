...
def read_files(f):
	df=pd.read_csv(f)
	df = df.loc[:,['categories','name','stars','full_address','latitude','longitude']]
	df = df.dropna()
...
with open('linked_cate_name.json','w') as outfile:
	# outfile.write('{')
	for key in new_dict:
		json.dump((key,new_dict[key]),outfile)#now we write bunchs of lists to
		#file instead of dictionary
		outfile.write('\n')
	# outfile.write('}')
